...
public class SendMoneyController {
    ...
    public void send(ActionEvent event) {
        ...
        try {
            ...
            String totp = totpCode.getText();
            if (totp != null){
                totp = totp.trim();
            }
            if(totp.isEmpty()){
                informationalAlert("Box alert", "You much input TOTP Code to send BTC.");
                return;
            }
            if(!totp.matches("^\\d+$") || totp.length() != 6){
                informationalAlert("Box alert", "TOTP Code invalid.");
                return;
            }
            int code = Integer.valueOf(totp);
            if(!na.verifyCode(code)){
                informationalAlert("Box alert", "TOTP Code invalid.");
                return;
            }
            // Send coin BTC.
            ...
            System.out.println("============ ECKey.KeyIsEncryptedException ============");
    ...
    private void retrySend() {
        ...
        final String totpStr = totpCode.getText();
...
//        screen.controller.aesKey = cur;
        ...
        screen.controller.totpCode.setText(totpStr);
