...
public class BitcoinAddressValidator {
    ...
    public BitcoinAddressValidator(NetworkParameters params, TextField fieldAddress, Node... nodes) {
        this.params = params;
        this.nodes = nodes;
        ...
        new TextFieldValidator(fieldAddress, text -> text.isEmpty() || testAddr(text));
        // However we do want the buttons to be disabled when empty so we apply a different test there.
        fieldAddress.textProperty().addListener((observableValue, prev, current) -> {
            toggleButtons(current);
        });
        toggleButtons(fieldAddress.getText());
    }
